...
public interface MeetingRepository extends JpaRepository<Meeting , Integer> {
    @Query("SELECT m FROM Meeting m WHERE m.relation_id.relation_id = :relationId ORDER BY m.start_day DESC, m.start_hour ASC")
    List<Meeting> findByRelationId(int relationId);

    @Query("SELECT m FROM Meeting m WHERE m.relation_id.relation_id IN :relationIds ORDER BY m.start_day DESC, m.start_hour ASC")
    ...
    @Modifying
    @Query("DELETE FROM Meeting m WHERE m.start_day < CURRENT_DATE - 30")
    void deleteMeetingsOlderThan30Days();
